package layouts;


public class Test {
    public static void main(String[] args) {
        /*TIPOS DE LAYOUT
         * 
         * FlowLayout(PorDefecto) -> Coloca los componentes de izquierda a derecha en el orden en el que los vamos añadiendo
         * BorderLayout -> Divide el contenedor que lo acoja en 5 zonas, norte sur este oeste y centro
         *      -Los contenedores de norte y sur ocupan el maximos espacio posible horizontal y el minimo verticalmente
         *      -Las zonas este y oeste se da el caso inverso
         *      -La zona central es la unica que crece por igual en todas direcciones
         * GridLayout -> Divide el contenedor en una matriz de filas y columnas
         * BoxLayout -> solo se puede aplicar sobre un tipo de contenedor, que es de tipo Box<>
         *      -Los controles que añadamos al box no modificaran su tamaño
         *      -Puede ser con una linea vertical o horizontal
         *      -Se pueden añadir espacios entre botones, tu dices el espacio entre botones
         * GridBackLayout -> Coloca componenter en filas y columnas, pero le puedes decir que agrupe filas/columnas si es necesario
         *      -Los componentes que se metan en las celdas ocuparan su totalidad
         * 
         * 
         *      -GridBagConstraints() en el .add
         *      -aaa.add(Component comp, GridBagCOnstraints constr)
         * GrouLayout ->
         * CardLayout ->
         */



    }
}
